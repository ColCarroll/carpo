...
class TestCarpo(object):
    ...
    def test_executes(self):
        ...
        self.runner.invoke(run, ['--db-file', self.pm.db_file, new_nb])
        # Now has been executed
        assert self.pm.get_output(new_nb) == '4'

    def test_executes_multiple(self):
        first_notebook = self.pm.add_nb_with_input('2 * 2')
        second_notebook = self.pm.add_nb_with_input('3 * 3')
        self.runner.invoke(run, ['--db-file', self.pm.db_file, first_notebook, second_notebook])
        # Now has been executed
    ...
    def test_executes_repeatedly(self):
        ...
        self.runner.invoke(run, ['--db-file', self.pm.db_file, new_nb])
        assert self.pm.get_output(new_nb) == '9'
    ...
    def test_exceptions_in_git(self):
        ...
        self.runner.invoke(run, ['--db-file', self.pm.db_file, new_nb])
        assert self.pm.get_output(new_nb) == '0'
